...
def build_model(input_dim, embedding_dim, maxlen):
    ...
    dense_out = layers.TimeDistributed(layers.Dense(25, activation="relu"))(output_sequences)

    mask = layers.Input(shape=(maxlen,), dtype=tf.bool)
    crf = CRF(4, name='crf')
    predicted_sequence, potentials, sequence_length, crf_kernel = crf(dense_out)

    model = models.Model(input_, potentials)
...
def evaluate_model(y_test, y_pred, class_names):
    ...
    plt.title("Bi-LSTM + CRF - Aspect Extraction")
    plt.savefig("./output/bilstm_crf_confusion_matrix.png")
