...
def parse_arguments():
    ...
    parser.add_argument("--glove-path", type=str, default="/mnt/d/Datasets/glove.twitter.27B.100d.txt", help="GloVe embeddings path")
...
def main():
    ...
    embeddings_index = {}
    f = open(args.glove_path, "r", encoding="utf-8")
    ...
    embedding_matrix = np.zeros((input_dim, args.embedding_dim))
    ...
    model = models.Sequential([
        ...
        layers.Embedding(len(tokenizer.word_index)+1, args.embedding_dim, weights=[embedding_matrix], input_length=args.maxlen, trainable=False),
    ...
    print(f"Bi-LSTM + GloVe Train Time = {model1_train_time:.4f}")
    ...
    print(f"Bi-LSTM + GloVe Test Time = {model1_test_time:.4f}")
    ...
    print(f"Bi-LSTM + GloVe Train Score = {model_train_score * 100:.4f}%")
    print(f"Bi-LSTM + GloVe Test Score = {model_test_score * 100:.4f}%")
    ...
    print(f"Bi-LSTM + GloVe Precision Score = {model_precision_score * 100:.4f}%")
    print(f"Bi-LSTM + GloVe F1 Score = {model_f1_score * 100:.4f}%")
    print(f"Bi-LSTM + GloVe Recall Score = {model_recall_score * 100:.4f}%")
    print(f"Bi-LSTM + GloVe Accuracy Score = {model_accuracy_score * 100:.4f}%")
