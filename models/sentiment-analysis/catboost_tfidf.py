...
def main():
    ...
    X_valid_tfidf = cv.transform(X_valid)
    ...
    catboost_tfidf_pred_train = xgb.predict(X_train_tfidf)
    catboost_tfidf_test_start = time.time()
    catboost_tfidf_pred_test = xgb.predict(X_test_tfidf)
    ...
    model_cm = confusion_matrix(y_test, catboost_tfidf_pred_test)
